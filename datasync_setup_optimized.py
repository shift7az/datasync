...
logging.Logger.success = lambda self, msg, *args: self._log(25, msg, args) if self.isEnabledFor(25) else None

COLORS = {'DEBUG': '\033[36m', 'INFO': '\033[34m', 'SUCCESS': '\033[32m', 
...
def load_config(path: Path) -> Dict:
    ...
    req = ['profiles.source', 'profiles.target', 'aws_region', 'datasync_role_name', 
           'iam_policy_name', 'migrations']
    missing = [f for f in req if not any(f.split('.')[0] in cfg and 
               (len(f.split('.')) == 1 or f.split('.')[1] in cfg.get(f.split('.')[0], {})) for _ in [1])]
    if missing: raise ValueError(f"Missing config fields: {missing}")
    return cfg

# ============================================
# BACKUP MANAGEMENT
# ============================================
def backup_policies(s3_src, s3_tgt, src_bucket: str, dst_bucket: str, backup_dir: Path,
                   src_prof: str, tgt_prof: str, log, dry_run: bool) -> None:
    """Backup bucket policies to timestamped dir. Creates restore.sh for one-click rollback."""
    if dry_run: return log.info("[DRY RUN] Would backup policies")
    ...
    (backup_dir / "restore.sh").write_text(f"""#!/bin/bash
set -e
...
# ============================================
def setup_iam(iam, role_name: str, policy_name: str, acct_id: str, region: str,
             src_buckets: List[str], dst_buckets: List[str], dst_acct: str,
             log, dry_run: bool) -> Tuple[str, bool]:
    ...
    iam.create_role(RoleName=role_name, AssumeRolePolicyDocument=json.dumps({
        ...
        "Statement": [{"Effect": "Allow", "Principal": {"Service": "datasync.amazonaws.com"},
                      ...
                      "Condition": {"StringEquals": {"aws:SourceAccount": acct_id},
    ...
    # Attach policy
    iam.put_role_policy(RoleName=role_name, PolicyName=policy_name, PolicyDocument=json.dumps({
        "Version": "2012-10-17",
        "Statement": [
            {"Sid": "SourceBucketAccess", "Effect": "Allow",
             "Action": ["s3:GetBucketLocation", "s3:ListBucket", "s3:ListBucketMultipartUploads"],
             "Resource": [f"arn:aws:s3:::{b}" for b in src_buckets]},
            {"Sid": "SourceObjectAccess", "Effect": "Allow",
             "Action": ["s3:GetObject", "s3:GetObjectTagging", "s3:GetObjectVersion", 
                       "s3:GetObjectVersionTagging", "s3:ListMultipartUploadParts"],
             "Resource": [f"arn:aws:s3:::{b}/*" for b in src_buckets]},
            {"Sid": "DestBucketAccess", "Effect": "Allow",
             "Action": ["s3:GetBucketLocation", "s3:ListBucket", "s3:ListBucketMultipartUploads"],
             "Resource": [f"arn:aws:s3:::{b}" for b in dst_buckets],
             "Condition": {"StringEquals": {"aws:ResourceAccount": dst_acct}}},
            {"Sid": "DestObjectAccess", "Effect": "Allow",
...
def update_policy(s3, bucket: str, role_arn: str, log, dry_run: bool) -> None:
    ...
    if dry_run: return log.info(f"[DRY RUN] Would update policy for {bucket}")
    
    ds_stmts = [
        {"Sid": "DataSyncAllowBucketAccess", "Effect": "Allow", "Principal": {"AWS": role_arn},
         "Action": ["s3:GetBucketLocation", "s3:ListBucket", "s3:ListBucketMultipartUploads"],
         "Resource": f"arn:aws:s3:::{bucket}"},
        {"Sid": "DataSyncAllowObjectAccess", "Effect": "Allow", "Principal": {"AWS": role_arn},
         "Action": ["s3:AbortMultipartUpload", "s3:DeleteObject", "s3:GetObject", 
                   "s3:PutObject", "s3:PutObjectTagging", "s3:GetObjectTagging"],
         "Resource": f"arn:aws:s3:::{bucket}/*"}
    ]
    ...
    try:
        ...
        pol = {"Version": "2012-10-17", "Statement": []} if e.response['Error']['Code'] == 'NoSuchBucketPolicy' else (_ for _ in ()).throw(e)
    ...
    pol['Statement'] = [s for s in pol.get('Statement', []) if s.get('Sid') not in ds_sids] + ds_stmts
        ...
        try:
            ...
            if code in ['Throttling', 'RequestLimitExceeded', 'TooManyRequestsException']:
                if attempt < max_retries - 1:
                    ...
                    if log: log.warning(f"Rate limited, retrying in {wait}s...")
                    time.sleep(wait)
                    continue
...
def create_datasync(ds, src: str, dst: str, role_arn: str, opts: Dict, log, dry_run: bool) -> str:
    ...
    try:
        ...
        src_loc = retry_with_backoff(
            lambda: ds.create_location_s3(S3BucketArn=f"arn:aws:s3:::{src}",
                                         S3Config={'BucketAccessRoleArn': role_arn},
                                         Subdirectory='/', S3StorageClass='STANDARD')['LocationArn'],
            log=log
        )
        ...
        dst_loc = retry_with_backoff(
            lambda: ds.create_location_s3(S3BucketArn=f"arn:aws:s3:::{dst}",
                                         S3Config={'BucketAccessRoleArn': role_arn},
                                         Subdirectory='/', S3StorageClass='STANDARD')['LocationArn'],
            log=log
        )
        ...
        task_opts = {'TransferMode': opts.get('TransferMode', 'CHANGED'),
        ...
        task_params = {'SourceLocationArn': src_loc, 'DestinationLocationArn': dst_loc,
                       'Name': f"Migration-{src}-to-{dst}", 'Options': task_opts}
        ...
        task_arn = retry_with_backoff(
            lambda: ds.create_task(**task_params)['TaskArn'],
            log=log
        )
        ...
        if created_resources:
            log.warning(f"Task creation failed, rolling back {len(created_resources)} resource(s)...")
...
def main():
    ...
    try:
        ...
        src_sess = boto3.Session(profile_name=cfg['profiles']['source'], region_name=cfg['aws_region'])
        tgt_sess = boto3.Session(profile_name=cfg['profiles']['target'], region_name=cfg['aws_region'])
        ...
        role_arn, created = setup_iam(iam, cfg['datasync_role_name'], cfg['iam_policy_name'],
                                      src_acct, cfg['aws_region'], src_buckets, dst_buckets,
                                      tgt_acct, log, args.dry_run)
            ...
            try:
                ...
                if not args.dry_run:
                    ...
                    if not verify_bucket_access(s3_tgt, mig['destination_bucket'], log):
                        raise ValueError(f"Cannot access destination bucket: {mig['destination_bucket']}")
                    log.success("Bucket access verified\n")
                
                # Backup policies to timestamped dir
                ts = datetime.now().strftime('%Y%m%d-%H%M%S')
                bak_dir = Path(cfg.get('backup_dir', Path.home() / 'Downloads' / 'backups')) / f"mig{idx}-{ts}"
                backup_policies(s3_src, s3_tgt, mig['source_bucket'], mig['destination_bucket'],
                               bak_dir, cfg['profiles']['source'], cfg['profiles']['target'], 
                               log, args.dry_run)
                ...
                task_arn = create_datasync(ds, mig['source_bucket'], mig['destination_bucket'],
                                          role_arn, mig.get('options', {}), log, args.dry_run)
